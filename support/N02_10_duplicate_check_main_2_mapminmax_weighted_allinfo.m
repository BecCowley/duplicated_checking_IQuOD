...
for nian=1:length(flist)
    ...
    filename=['./potential_duplicates_output/potential_duplicate_',num2str(nian),'_mapminmax_weight_allinfo.txt']
    if(exist(filename))
        delete(filename)
    end
    ...
    for i=1:length(average_DNA)
        ...
        duplicate_number=sum(difference<0.001);   % threshold value: 0.0001%
