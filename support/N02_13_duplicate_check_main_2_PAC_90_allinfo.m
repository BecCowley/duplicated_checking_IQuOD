...
for nian=1:length(flist)
    ...
    s=0;
    i=0;
    while s/sum(eig1)<0.9   % set the principal component contribution threshold to 90%
        i=i+1;
        s=s+eig1(i);
    end
    DNA_new=z*v(:,1:i);
    ...
    filename=['./potential_duplicates_output/potential_duplicate_',num2str(nian),'_PCA_90_allinfo.txt'];
    if(exist(filename))
        delete(filename)
    end
